  ...
  const getData = async () => {
    try {
      const freezerValue = await AsyncStorage.getItem('freezer-key').then(
        (freezerJson) => freezerJson && JSON.parse(freezerJson)
      );
      const refrigeratorValue = await AsyncStorage.getItem(
        'refrigerator-key'
      ).then((refrigeratorJson) => refrigeratorJson && JSON.parse(refrigeratorJson));
      const pantryValue = await AsyncStorage.getItem('pantry-key').then(
        (pantryJson) => pantryJson && JSON.parse(pantryJson)
      );
      const counterValue = await AsyncStorage.getItem('counter-key').then(
        (counterJson) => counterJson && JSON.parse(counterJson)
      );
